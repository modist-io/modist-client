...
try:
    ...
    raise KeyError("cannot run invoke tasks if setup.cfg is missing [metadata] section")
...
def test(ctx):
    ...
    report.success(ctx, "test", f"testing project {ctx.metadata['name']!r}")
...
def publish(ctx, test=False, force=False, draft=False):
    ...
    try:
        ...
        report.info(ctx, "publish", "pushing git tags")
        if not draft:
        ...
        git_reset_command = "git reset --soft HEAD^"
...
namespace = invoke.Collection(
    build, clean, test, lint, publish, docs, package, profile, linter
)
namespace.configure(
    {
        "metadata": metadata,
