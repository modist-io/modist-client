...
for working on the Modist client package.
...
Now that you have ``pyenv`` we can install the necessary Python version. The Modist
client package depends on Python 3.7+, so we can request that through ``pyenv``.
...
.. code-block:: console
   ...
   $ pwd
   /PATH/TO/CLONED/REPOSITORY/modist-client
   $ pyenv local 3.7  # to mark the modist-client directory as needing Python 3.7+
   ...
...
We use `Pipenv <https://pipenv-fork.readthedocs.io/en/latest/>`_ to manage both our
dependencies and virtual environments. Setting up ``pipenv`` just involves installing
it through ``pip`` as a user-level dependency.

.. code-block:: console

   $ pip install --user pipenv
   Collecting pipenv
   Downloading pipenv-x.x.x-py2.py3-non-any.whl
   ...

   $ pipenv --version
   pipenv, version xxxx.xx.xx
...
From within the Pipenv subshell, you can access and run these commands through the
